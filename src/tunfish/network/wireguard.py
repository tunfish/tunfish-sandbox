...
#from lib.network import parse_key, parse_net
...
class WireGuard(GenericNetlinkSocket):
    ...
    def get_devices(self):
        ip = IPRoute()
        links = [link.get_attr('IFLA_IFNAME') for link in ip.get_links() if
                 self.device_exists(link.get_attr('IFLA_IFNAME'))]
    ...
    def set_device(self, ifname=None, ifindex=None, config={}):
        ...
        if pkey != None:
            msg['attrs'].append(['WGDEVICE_A_PRIVATE_KEY', b64decode(pkey)])
            ...
            for peer in peers:
                ...
                if 'preshared_key' in peer:
                    if len(peer['preshared_key']) != 64:
                        ...
                        print('pre-shared key must be 64 hexadecimal characters')
                ...
                if 'persistent_keepalive_interval' in peer:
                    try:
                        wgpeer['attrs'].append(
                            ['WGPEER_A_PERSISTENT_KEEPALIVE_INTERVAL', int(peer['persistent_keepalive_interval'])])
                    except ValueError:
                        pass
                ...
                if 'public_key' in peer:
                    ...
                    wgpeer['attrs'].append(['WGPEER_A_PUBLIC_KEY', b64decode(peer['public_key'])])
